...
class MusicBot(discord.Client):
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            return Response(helpmsg, reply=True, delete_after=6000)
    ...
    async def cmd_blacklist(self, message, user_mentions, option, something):
        ...
        Blacklisted users are forbidden from using bot commands. (Bot created by: Opsnuiven)
        """
    ...
    async def cmd_id(self, author, user_mentions):
        ...
        """
        if not user_mentions:
            return Response('your id is `%s` Bot created by: Opsnuiven' % author.id, reply=True, delete_after=35)
        else:
            usr = user_mentions[0]
            return Response("%s's id is `%s` Bot created by: Opsnuiven" % (usr.name, usr.id), reply=True, delete_after=35)
    ...
    async def cmd_summon(self, channel, author, voice_channel):
        ...
        if not author.voice_channel:
            raise exceptions.CommandError('You are not in a voice channel! (Bot created by: Opsnuiven)')
